...
class AppPathsConfig {
  final Map<String, String> defaultPaths;
  final Map<String, List<String>> searchPaths;
  final Map<String, CustomAppInfo> customApps;

  const AppPathsConfig({
  ...
  factory AppPathsConfig.fromMap(Map<String, dynamic> map) {
    return AppPathsConfig(
      defaultPaths: Map<String, String>.from(
        map['defaultPaths'] ?? <dynamic, dynamic>{},
      ),
      searchPaths:
          (map['searchPaths'] as Map<String, dynamic>? ?? <String, dynamic>{})
              .map(
                (String key, value) => MapEntry(key, List<String>.from(value)),
              ),
      customApps:
          (map['customApps'] as Map<String, dynamic>? ?? <String, dynamic>{})
              .map(
  ...
  String? getExecutablePath(String appName) {
    ...
    if (defaultPaths.containsKey(lowerName)) {
      return defaultPaths[lowerName];
    }

    // Check search paths
    if (searchPaths.containsKey(lowerName)) {
      return _findExecutableInPaths(searchPaths[lowerName]!);
    }

    // Check custom apps
    if (customApps.containsKey(lowerName)) {
      return _findExecutableInPaths(customApps[lowerName]!.paths);
    }
