...
/// Factory for creating launcher widgets with advanced styling support
class LauncherWidgetFactory extends WidgetFactory {
  ...
  Decoration? _buildAdvancedDecoration(Map<String, dynamic> style) {
    ...
    final Map<String, dynamic>? borderMap =
        style['border'] as Map<String, dynamic>?;
    if (borderMap != null) {
    ...
    final Map<String, dynamic>? shadowMap =
        style['shadow'] as Map<String, dynamic>?;
    if (shadowMap != null) {
  ...
  /// Handle launcher tap action
  void _onLauncherTap(
    String? launcherRef,
    String actionType,
    Map<String, dynamic> overrides,
  ) {
    debugPrint('Launcher tapped: $launcherRef, action: $actionType');

    if (launcherRef == null) return;

    // Get launcher from config service
    final LauncherItem? launcher = configService?.getLauncher(launcherRef);
    if (launcher == null) {
      debugPrint('Launcher not found: $launcherRef');
      return;
    }
  ...
  /// Get icon for specific launcher
  IconData _getIconForLauncher(String launcherRef) {
    switch (launcherRef.toLowerCase()) {
      case 'steam':
